...
What works:
1) Your PC shows up via ADDP - so Digi Device Discovery utility will see and display it
2) Your PC uses EDP to iDigi, which with ADDP support means you can add your PC to your iDigi account using the web console. (Note: since the cp4pc will refuse to 'reboot', you'll get a warning during this addition - but it works)
...
5) Your main script needs to import xbee.py as a whole, which adds the Digi XBee socket extensions to your PC's socket module.

6) If you are running Dia, then until Dia 2.0.x is released you will see an exception thrown because the xbee_device_manager.py uses one instance of the old constant socket.ZBS_PROT_TRANSPORT, which you can rename to the correct socket.XBS_PROT_TRANSPORT.
